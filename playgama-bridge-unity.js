...
const PREFS_SNAPSHOT_KEY = 'playgama_player_prefs_snapshot'
...
window.PLAYGAMA_PREFS_SNAPSHOT = ''
const messageQueue = []
...
window.getPlaygamaPreloadedPrefsSnapshot = function() {
    return window.PLAYGAMA_PREFS_SNAPSHOT || ''
}

function preloadPlayerPrefsSnapshot() {
    if (!bridge.storage) {
        return Promise.resolve()
    }

    try {
        let storageType = bridge.storage.defaultType
        if (!bridge.storage.isAvailable(storageType)) {
            if (bridge.storage.isAvailable('platform_internal')) {
                storageType = 'platform_internal'
            } else if (bridge.storage.isAvailable('local_storage')) {
        ...
        return bridge.storage.get([PREFS_SNAPSHOT_KEY], storageType, false)
            .then(data => {
                if (Array.isArray(data) && data.length > 0 && typeof data[0] === 'string') {
                    window.PLAYGAMA_PREFS_SNAPSHOT = data[0]
                    return
                }
        ...
        console.warn('Playgama prefs preload skipped:', error)
        return Promise.resolve()
    }
}
...
function initializeBridge() {
    ...
    bridge
        ...
        .then(() => {
            ...
            return preloadPlayerPrefsSnapshot()
        })
        .then(() => {

            let unityLoader = document.createElement('script')
            unityLoader.src = 'Build/VK_KingCrashText_ncu.loader.js'
            unityLoader.onload = () => {
                createUnityInstance(
                    CANVAS,
                    {
                        dataUrl: 'Build/VK_KingCrashText_ncu.data.unityweb',
                        frameworkUrl: 'Build/VK_KingCrashText_ncu.framework.js.unityweb',
                        codeUrl: 'Build/VK_KingCrashText_ncu.wasm.unityweb',
                        streamingAssetsUrl: 'StreamingAssets',
                        companyName: 'DefaultCompany',
                        productName: 'NewCarUVC',
                        productVersion: '0.1',
